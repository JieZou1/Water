...
namespace WaterLib
{
    ...
    {
        /// <summary>
        /// Detect Zhong Chong using HoG+SVM. The results have been checked with overlappings and have sorted according to scores.
        /// </summary>
        /// <param name="image"></param>
        /// <returns></returns>
        private List<DetectResult> DetectZhongWithRotationModels(Image<Gray, byte> image)
        {
            //resize the image
            double scale = winSize_64.Height / 64.0; //check statistics.txt to decide this scaling factor.
            //Image<Gray, byte> img_scaled = image.Resize(scale, Emgu.CV.CvEnum.Inter.Linear);
            Image<Gray, byte> img_scaled = image; //Scale is 1.
            ...
            for (int deg = 0; deg < 360; deg += 30)
            {
                switch (deg)
                {
                    case 0: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_0); id = 0; break;
                    case 30: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_30); id = 30; break;
                    case 60: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_60); id = 60; break;
                    case 90: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_90); id = 90; break;
                    case 120: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_120); id = 120; break;
                    case 150: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_150); id = 150; break;
                    case 180: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_180); id = 180; break;
                    case 210: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_210); id = 210; break;
                    case 240: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_240); id = 240; break;
                    case 270: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_270); id = 270; break;
                    case 300: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_300); id = 300; break;
                    case 330: hog_detection.SetSVMDetector(svm_model_zhong_chong_64_330); id = 330; break;
                }
            ...
            for (int i = 0; i < total_objs.Count; i++)
            {
                ...
                //if (orig_rect.Width < 50 || orig_rect.Height < 50) continue;
