...
namespace WaterLib
{
    ...
    {
        private List<DetectResult> DetectDanZhiLunWithRotationModels(Image<Gray, byte> image)
        {
            //resize the image
            double scale = winSize_64.Height / 288.0; //check statistics.txt to decide this scaling factor.
            ...
            //0 degree model
            hog_detection.SetSVMDetector(svm_model_danzhilun_chong_64_0);
            ...
            //45 degree model
            hog_detection.SetSVMDetector(svm_model_danzhilun_chong_64_45);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //90 degree model
            hog_detection.SetSVMDetector(svm_model_danzhilun_chong_64_90);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //135 degree model
            hog_detection.SetSVMDetector(svm_model_danzhilun_chong_64_135);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //180 degree model
            hog_detection.SetSVMDetector(svm_model_danzhilun_chong_64_180);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //225 degree model
            hog_detection.SetSVMDetector(svm_model_danzhilun_chong_64_225);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //270 degree model
            hog_detection.SetSVMDetector(svm_model_danzhilun_chong_64_270);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            // 315 degree model
            hog_detection.SetSVMDetector(svm_model_danzhilun_chong_64_315);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);
            ...
            for (int i = 0; i < total_objs.Count; i++)
            {
                ...
                if (orig_rect.Width > 370 || orig_rect.Height > 370) continue;
