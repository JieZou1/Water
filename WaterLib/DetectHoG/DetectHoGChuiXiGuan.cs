...
namespace WaterLib
{
    ...
    {
        private List<DetectResult> DetectChuiXiGuanWithRotationModels(Image<Gray, byte> image)
        {
            //resize the image
            double scale = winSize_64.Height / 208.0; //check statistics.txt to decide this scaling factor.
            ...
            //0 degree model
            hog_detection.SetSVMDetector(svm_model_chuixiguan_chong_64_0);
            ...
            //45 degree model
            hog_detection.SetSVMDetector(svm_model_chuixiguan_chong_64_45);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //90 degree model
            hog_detection.SetSVMDetector(svm_model_chuixiguan_chong_64_90);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //135 degree model
            hog_detection.SetSVMDetector(svm_model_chuixiguan_chong_64_135);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //180 degree model
            hog_detection.SetSVMDetector(svm_model_chuixiguan_chong_64_180);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //225 degree model
            hog_detection.SetSVMDetector(svm_model_chuixiguan_chong_64_225);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            //270 degree model
            hog_detection.SetSVMDetector(svm_model_chuixiguan_chong_64_270);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);

            // 315 degree model
            hog_detection.SetSVMDetector(svm_model_chuixiguan_chong_64_315);
            objects = hog_detection.DetectMultiScale(img_scaled, hitThreshold, winStride, testPadding, scaleStep, groupThreshold, useMeanShiftGrouping);
            total_objs.AddRange(objects);
            ...
            for (int i = 0; i < total_objs.Count; i++)
            {
                ...
                //if (orig_rect.Width < 180 || orig_rect.Height < 180) continue;
                if (orig_rect.Width > 550 || orig_rect.Height > 550) continue;
